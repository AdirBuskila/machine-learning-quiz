"use strict";
/* Machine-Learning quiz — vanilla JS. Loads window.QUESTIONS from questions.js. */

const QS = Array.isArray(window.QUESTIONS) ? window.QUESTIONS : [];
const HE_KEYS = ["א","ב","ג","ד","ה","ו","ז","ח"];
const STORE = "mlq_progress_v1";
...
function initStart(){
  ...
  $("#datasetInfo").textContent = `${QS.length} שאלות · מבחני 2022–2026 + תרגול`;
}
...
function renderQuestion(){
  ...
  // reuse stored view in exam mode so shuffle is stable when navigating back
  if(S.mode==="exam"){
  ...
  // exam: restore previous selection
  ...
  if(S.mode==="exam"){
    ...
    $("#nextBtn").classList.add("hidden");      // appears after answering
...
function next(){
  if(S.mode==="practice"){
    if(!S.current.answered) return;          // must answer first
    if(S.pos>=S.pool.length-1){ // refill: keep practicing
      S.pool = shuffle(S.pool); S.pos=0;
    } else S.pos++;
    renderQuestion();
  } else {
